...
from .models import Visitor, PageCOunt
...
class AddListAPIView(generics.ListAPIView):
    queryset = models.Advertisement.objects.all().order_by("?")[:1]
...
class NewsRetrieveAPIView(generics.RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        models.News.objects.filter(pk=instance.pk).update(views=F('views') + 1)
        return super().retrieve(request, *args, **kwargs)

    

class NewsListAPIView(BasicNewsListAPIView):
    queryset = models.News.objects.all().select_related('category', 'region').order_by("-published_at")
    serializer_class = serializers.NewsListSerializer
    pagination_class = pagination.PageTwenty 
...
class ContactCreateViewSet(generics.CreateAPIView):
    queryset = models.ContactCreate.objects.all()
    serializer_class = serializers.ConactCreateSerializer
